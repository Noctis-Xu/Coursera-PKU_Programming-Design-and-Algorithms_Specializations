...
using namespace std;
int n, m;//迷宫n行m列
int shortest_steps;//用来记录已知的最短路线，对应要点7
...
int recursion(char* p[], int r, int c, int steps = 0) {//p表示当前迷宫；r,c表示这次的位置，r为行，c为列，对应要点2。steps是从入口S到当前位置的步数，每调用一次recursion，就加一步
	...
	if (c > 0 && (p[r][c - 1] == '.' || p[r][c - 1] == 'T')) {//进行左移，前提是不在边界以及不碰墙壁。对应要求中的1，4和5
		//steps++;//不能这样写，因为后面走上走右走下还要用到steps，故不能修改其值。r,c同理
		if (p[r][c - 1] == 'T') {//判断到达终点
			if (steps + 1 <= shortest_steps) {//更新最短路线
				shortest_steps = steps + 1;
			}
			steps_left = 1;//递归函数的出口
		}
...
int main() {
	cin >> n >> m;
	shortest_steps = n * m - 1;//最短路线步数最长的情况（不走回头路）
